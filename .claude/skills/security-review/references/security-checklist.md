...
### SQL Injection
- [ ] [CRITICAL] `f"SELECT ` / `f"INSERT ` / `f"UPDATE ` / `f"DELETE ` 패턴 없음
  - Grep: `f["'].*\b(SELECT|INSERT|UPDATE|DELETE|DROP)\b`
- [ ] [CRITICAL] `.format()` + SQL 키워드 결합 없음
  - Grep: `\.format\(.*\).*(SELECT|INSERT|UPDATE|DELETE)`
- [ ] [HIGH] `text()` 사용 시 파라미터 바인딩 적용 (f-string + text() 결합 금지)
  - Grep: `text\(f["']`
- [ ] [MEDIUM] `exec()` / `eval()` 사용 없음
  - Grep: `\bexec\s*\(|\beval\s*\(`

### Command Injection
- [ ] [CRITICAL] `subprocess.*(shell=True)` 사용 없음
  - Grep: `subprocess\.\w+\(.*shell\s*=\s*True`
- [ ] [CRITICAL] `os.system(` 사용 없음
  - Grep: `os\.system\s*\(`
- [ ] [HIGH] `os.popen(` 사용 없음
  - Grep: `os\.popen\s*\(`

### Template Injection
- [ ] [MEDIUM] Jinja2 사용 시 `autoescape=True` 설정
...
### 엔드포인트 보호
- [ ] [CRITICAL] POST/PUT/DELETE 엔드포인트에 인증 dependency 존재
  - Grep: `@router\.(post|put|delete|patch)` → 해당 함수에 `Depends(.*auth|.*current_user|.*token)` 확인
  - 참고: 인증 미구현 상태이면 주의 항목으로 기록
- [ ] [HIGH] Admin 엔드포인트 접근 제한 확인
  - Grep: `interface/admin/` 파일에 인증 설정 확인

### 자격 증명 관리
- [ ] [CRITICAL] 하드코딩된 password/secret/api_key/token 없음
...
### 암호화
- [ ] [MEDIUM] 비밀번호 해싱 사용 (bcrypt, argon2 등)
...
### 취약 의존성
- [ ] [HIGH] `pip audit` 또는 `uv pip audit` 실행 결과 취약점 없음
  - 실행: `uv pip audit 2>/dev/null || pip audit 2>/dev/null || echo "audit tool 미설치"`

### 디버그 모드
- [ ] [CRITICAL] Production에서 debug=True 비활성화
  - Grep: `debug\s*=\s*True` (조건문 없이 직접 설정된 경우)
- [ ] [HIGH] Production에서 docs/swagger 비활성화
  - config.py의 `docs_url`이 `is_dev` 조건 사용 확인

### CORS 설정
- [ ] [HIGH] Production에서 `allow_origins=["*"]` 미사용
  - config.py의 `allow_origins`이 `is_dev` 조건으로 분기 확인
- [ ] [MEDIUM] `allow_methods=["*"]`, `allow_headers=["*"]` 범위 검토

### 시크릿 관리
- [ ] [CRITICAL] `.env` 파일이 `.gitignore`에 포함
  - .gitignore에서 `\.env` 패턴 존재 확인
- [ ] [HIGH] 설정 값이 환경변수에서 로딩 (하드코딩 아님)
  - Settings 클래스에서 `validation_alias` 사용 확인
- [ ] [MEDIUM] Field default 값에 실제 시크릿 미포함
